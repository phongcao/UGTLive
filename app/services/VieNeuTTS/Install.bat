...
echo [Step 3/4] Installing FastAPI and Uvicorn...
echo [Step 3/4] Installing FastAPI/Uvicorn... >> "%LOG_FILE%"
python -m pip install fastapi "uvicorn[standard]" certifi numpy >> "%LOG_FILE%" 2>&1
if errorlevel 1 (
...
python -c "from vieneu import Vieneu; print('VieNeu-TTS SDK imported successfully')" >> "%LOG_FILE%" 2>&1
if errorlevel 1 (
    echo WARNING: VieNeu-TTS SDK verification failed - model will download on first use
    echo WARNING: VieNeu-TTS SDK verification failed >> "%LOG_FILE%"
) else (
    echo VieNeu-TTS SDK verification passed >> "%LOG_FILE%"
)
...
echo.
echo Next steps:
echo   1. Run RunServer.bat to start the service
echo   2. Run TestService.bat to test the running service
echo.
